...
color_cycle = cycle(['light blue', 'light green', 'light pink', 'light yellow', 'cyan'])
egg_width = 45
egg_height = 55
egg_score = 10
egg_speed = 500
...
lives_text = c.create_text(canvas_width -10, 10, anchor = 'ne', font = game_font, fill = 'darkblue', \
                           text = 'Henget: ' + str (lives_remaining))
eggs = []
...
def check_catch():
    ...
    for egg in eggs:
        (egg_x, egg_y, egg_x2, egg_y2 ) = c.coords(egg)
        if catcher_x < egg_x and egg_x2 < catcher_x2 and catcher_start_y2 - egg_y2 < 40:
            eggs.remove(egg)
            c.delete(egg)
